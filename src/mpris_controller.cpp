...
QString MprisController::playbackStatusString(bool isPlaying, bool hasTrack)
{
    ...
    if (hasTrack) return QStringLiteral("Paused");
...
int MprisController::loopStatusToRepeatMode(const QString &loopStatus)
{
    ...
    if (loopStatus.compare(QLatin1String("Playlist"), Qt::CaseInsensitive) == 0) return 1;
    return 0;
}
...
QVariantMap MprisController::buildMetadata(const QVariantMap &track, qint64 durationMs)
{
    ...
    if (track.isEmpty()) {
#ifdef CASSETTECAT_HAVE_DBUS
        meta.insert(QStringLiteral("mpris:trackid"), QVariant::fromValue(QDBusObjectPath(QStringLiteral("/org/mpris/MediaPlayer2/TrackList/NoTrack"))));
#else
...
#ifdef CASSETTECAT_HAVE_DBUS
    meta.insert(QStringLiteral("mpris:trackid"), QVariant::fromValue(QDBusObjectPath(QStringLiteral("/org/mpris/MediaPlayer2/Track/Current"))));
#else
    ...
    if (!artworkUrl.isEmpty()) {
        if (artworkUrl.startsWith(QLatin1String("http://")) ||
            artworkUrl.startsWith(QLatin1String("https://")) ||
...
bool MprisController::selfCheck()
{
    ...
    if (playbackStatusString(false, true) != QLatin1String("Paused")) return false;
    if (playbackStatusString(false, false) != QLatin1String("Stopped")) return false;

    if (loopStatusString(0) != QLatin1String("None") || loopStatusToRepeatMode(QStringLiteral("None")) != 0) return false;
    if (loopStatusString(1) != QLatin1String("Playlist") || loopStatusToRepeatMode(QStringLiteral("Playlist")) != 1) return false;
    if (loopStatusString(2) != QLatin1String("Track") || loopStatusToRepeatMode(QStringLiteral("Track")) != 2) return false;

    const QVariantMap emptyMeta = buildMetadata(QVariantMap(), 0);
#ifdef CASSETTECAT_HAVE_DBUS
    if (emptyMeta.value(QStringLiteral("mpris:trackid")).value<QDBusObjectPath>().path() != QLatin1String("/org/mpris/MediaPlayer2/TrackList/NoTrack")) {
        return false;
    }
#else
    if (emptyMeta.value(QStringLiteral("mpris:trackid")).toString() != QLatin1String("/org/mpris/MediaPlayer2/TrackList/NoTrack")) {
        return false;
    }
#endif
...
#ifdef CASSETTECAT_HAVE_DBUS
    if (fullMeta.value(QStringLiteral("mpris:trackid")).value<QDBusObjectPath>().path() != QLatin1String("/org/mpris/MediaPlayer2/Track/Current")) return false;
#else
    if (fullMeta.value(QStringLiteral("mpris:trackid")).toString() != QLatin1String("/org/mpris/MediaPlayer2/Track/Current")) return false;
#endif
    if (fullMeta.value(QStringLiteral("xesam:title")).toString() != QLatin1String("Test Song")) return false;
    if (fullMeta.value(QStringLiteral("xesam:album")).toString() != QLatin1String("Test Album")) return false;
    if (fullMeta.value(QStringLiteral("xesam:artist")).toStringList() != QStringList{QStringLiteral("Test Artist")}) return false;
    if (fullMeta.value(QStringLiteral("mpris:length")).toLongLong() != 120000000LL) return false;
    if (fullMeta.value(QStringLiteral("mpris:artUrl")).toString() != QLatin1String("https://example.com/cover.jpg")) return false;
    if (fullMeta.value(QStringLiteral("xesam:url")).toString() != QUrl::fromLocalFile(QStringLiteral("/music/test.flac")).toString()) return false;
