...
void ServicesController::searchAlbumCovers(const QString &album, const QString &artist)
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, requestToken, term, album, artist]() {
        ...
        auto queryArchive = [this, requestToken, album, artist](QVariantList res, QSet<QString> seen) {
            ...
            mbReq.setHeader(QNetworkRequest::UserAgentHeader, "CassetteCat/2.0.0 (https://github.com/samyyy2311/CassetteCat)");
            ...
            *sendMbRequest = [this, mbReq, requestToken, res, seen, weakSendMbRequest](int retryCount) mutable {
                ...
                QTimer::singleShot(delayMs, this, [this, mbReq, requestToken, res, seen, nextCallback, weakSendMbRequest, retryCount]() mutable {
                    if (!onlineEnabled() || requestToken != m_coverRequestToken) return;
                    s_lastMbRequestMs = QDateTime::currentMSecsSinceEpoch();

                    auto *mbReply = trackReply(m_net->get(mbReq));
                    connect(mbReply, &QNetworkReply::finished, this, [this, mbReply, requestToken, res, seen, nextCallback, weakSendMbRequest, retryCount]() mutable {
                        mbReply->deleteLater();
                        if (!onlineEnabled() || requestToken != m_coverRequestToken) return;

                        const int statusCode = mbReply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
                        if (statusCode == 503 && retryCount < 2) {
                            if (auto retryCb = weakSendMbRequest.lock()) {
                                QTimer::singleShot(1100, this, [retryCb, retryCount]() {
                                    (*retryCb)(retryCount + 1);
                                });
                            }
                            return;
                        }
                        ...
                        if (mbReply->error() == QNetworkReply::NoError) {
                            ...
                            for (const auto &val : releases) {
                                ...
                                if (!caaObj.value("front").toBool()) continue;

                                const QString mbid = obj.value("id").toString();
                                if (mbid.isEmpty()) continue;

                                const QString fullUrl = "https://coverartarchive.org/release/" + mbid + "/front";
                                if (seen.contains(fullUrl)) continue;
                                seen.insert(fullUrl);

                                const QString thumbUrl = "https://coverartarchive.org/release/" + mbid + "/front-500";

                                QString releaseYear;
                                const QString dateStr = obj.value("date").toString();
                                if (dateStr.length() >= 4) releaseYear = dateStr.left(4);
        ...
        connect(deezerReply, &QNetworkReply::finished, this, [this, deezerReply, requestToken, results, seenUrls, queryArchive]() mutable {
