...
QStringList MediaPlayer2Adaptor::supportedMimeTypes() const {
    return {QStringLiteral("audio/aac"),      QStringLiteral("audio/flac"),   QStringLiteral("audio/mpeg"),
            ...
            QStringLiteral("audio/wav"),      QStringLiteral("audio/x-aiff"), QStringLiteral("audio/x-m4a"),
...
void MediaPlayer2PlayerAdaptor::Seek(qlonglong offsetUs) {
    ...
    const qint64 targetMs = currentPos + offsetMs;
    if (durationMs > 0 && targetMs > durationMs) {
        Next();
        return;
    }
    const qint64 clampedMs = std::max<qint64>(0LL, targetMs);
    m_player->seek(clampedMs);
...
void *createLinuxMprisBackend(MprisController *controller, PlayerController *player) {
    ...
    if (!bus.registerService(serviceName)) {
        const QString fallback =
            QStringLiteral("org.mpris.MediaPlayer2.CassetteCat.instance%1").arg(QCoreApplication::applicationPid());
        if (!bus.registerService(fallback)) {
            qWarning() << "[MPRIS] Failed to register service" << serviceName << "and fallback" << fallback << ":"
...
void notifyLinuxMprisPlayingChanged(void *backend) {
    auto *b = static_cast<LinuxMprisBackend *>(backend);
    if (b && b->playerAdaptor) {
        b->playerAdaptor->notifyPropertiesChanged(
            {{QStringLiteral("PlaybackStatus"), b->playerAdaptor->playbackStatus()}});
    }
}
